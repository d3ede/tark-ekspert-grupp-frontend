...
import { ArrowDownRight,Check,MoveRight,Phone } from "lucide-react";
...
export function HomePage({locale}:{locale:Locale}){
  ...
  return <main>
    ...
    <section className="hero" id="top"><SiteHeader copy={copy}/><div className="hero-image"><Image src="/assets/construction-hero-winter.png" alt="" fill priority sizes="100vw"/></div><div className="hero-wash"/><div className="hero-content page-shell"><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.heroTitle}</h1><p className="hero-lead">{copy.heroText}</p><div className="hero-actions"><Link className="primary-button" href="#kontakt">{copy.heroPrimary}<MoveRight size={18}/></Link><Link className="ghost-button" href="#objektid">{copy.heroSecondary}<ArrowDownRight size={18}/></Link></div><div className="hero-proof">{copy.proof.map(item=><span key={item}><Check/>{item}</span>)}</div></div></section>
